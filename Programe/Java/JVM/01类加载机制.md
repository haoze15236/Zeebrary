...
![类运行流程图](https://gitee.com/Zeebrary/PicBed/raw/master/img/java/jvm/类运行流程图.png)



# 类加载过程

classLoader.loadClass(className)的类加载过程有如下几步：
...
参见[流程图](#流程图)可知其中会创建JVM启动器实例sun.misc.Launcher。
...
3. 如果父加载器及bootstrap类加载器都没有找到指定的类，那么调用当前类加载器(URLClassLoader)的findClass方法来完成类加载。
...
自定义类加载器只需要继承 java.lang.ClassLoader 类，该类有两个核心方法，一个是loadClass(String, boolean)，实现了**双亲委派机制**，还有一个方法是findClass，默认实现是空方法，所以我们自定义类加载器主要是**重写**findClass方法。
...
由于沙箱安全机制，不允许加载由自定义类加载器加载`java.lang`等java的核心包，所以需要修改`loadClass`方法，判断指定路径下的类使用自定义类加载器。
...
![img](https://note.youdao.com/yws/public/resource/35faf7c95e69943cdbff4642fcfd5318/xmlnote/B5AD127BCF044C05B44783F1D6DBC7E6/78905)
